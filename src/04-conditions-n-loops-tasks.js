...
function findFirstSingleChar(str) {
  ...
  const result = array.find((char, _, arr) => arr.indexOf(char) === arr.lastIndexOf(char));
  return result || null;
}
...
function isCreditCardNumber(ccn) {
  const cardNumber = String(ccn).replace(/\D/g, '');
  const digits = Array.from(cardNumber, Number);
  const sum = digits.reduceRight((acc, digit, index) => {
    const isOddIndex = (digits.length - 1 - index) % 2 === 1;
    let currentDigit;
    if (isOddIndex) {
      const doubledDigit = digit * 2;
      if (doubledDigit > 9) {
        currentDigit = doubledDigit - 9;
      } else {
        currentDigit = doubledDigit;
      }
    } else {
      currentDigit = digit;
    }
...
function isBracketsBalanced(str) {
  ...
  const checkBalanced = (index, stack) => {
    if (index >= str.length) {
...
function getCommonDirectoryPath(pathes) {
  const directories = pathes.map((path) => path.split('/').slice(0, -1));
  const commonPath = directories.reduce(
    (common, current) => common.filter((dir, i) => dir === current[i]),
  );
  return commonPath.concat('').join('/');
}
...
function getMatrixProduct(m1, m2) {
  const isProductPossible = m1.length > 0 && m2.length > 0 && m1[0].length === m2.length;

  if (!isProductPossible) {
    return [];
  }
...
function evaluateTicTacToePosition(position) {
  ...
  // Check columns
  for (let y = 0; y < 3; y += 1) {
    if (position[0][y] === position[1][y] && position[1][y] === position[2][y]) {
      if (position[0][y]) {
        return position[0][y];
      }
    }
